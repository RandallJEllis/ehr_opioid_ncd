...
def import_data():
    ...
    common_icd = set(sud_icd.CODE).intersection(aud_icd.CODE)
    sud_icd = sud_icd[~sud_icd.CODE.isin(common_icd)]
    #remove TOBACCO ICDs from sud_icd
    common_icd = set(sud_icd.CODE).intersection(tobacco_icd.CODE)
    sud_icd = sud_icd[~sud_icd.CODE.isin(common_icd)]
...
def build_population(sickle_icd_fu_mrns, hepc_icd_fu_mrns, hiv_icd_fu_mrns, aud_icd_fu_mrns, tobacco_icd_fu_mrns, sud_icd_fu_mrns,  depression_icd_fu_mrns,\
    ...
    pop['hepc'] = pop.MRN.isin(hepc_icd_fu_mrns)
