...
num_encounters_threshold = 5 #number of records each included patient must have
...
#                         ncd_age_threshold = 365.25*ncd_thresh
                ...
                control_cohort_remove_morethan2opioid_mrns = []
                for mrn in mrn_enrollment_keys:
                    if len(mrn_enrollment[mrn]) > 2:
                        control_cohort_remove_morethan2opioid_mrns.append(mrn)
...
#                 (~person.MRN.isin(control_cohort_remove_morethan2opioid_mrns)) &    
...
#                                            (person.YOB>1940)]
                ...
                pop['hepc'] = pop.MRN.isin(hepc_sub.MRN)
                pop['hepc'] = pop['hepc'].astype(int)

                hiv_sub = hiv_icd[hiv_icd.DIAGNOSIS_DATE<followup]
                pop['hiv'] = pop.MRN.isin(hiv_sub.MRN)
                pop['hiv'] = pop['hiv'].astype(int)

                aud_sub = aud_icd[aud_icd.DIAGNOSIS_DATE<followup]
                pop['aud'] = pop.MRN.isin(aud_sub.MRN)
                pop['aud'] = pop['aud'].astype(int)

                sud_sub = sud_icd[sud_icd.DIAGNOSIS_DATE<followup]
                pop['sud'] = pop.MRN.isin(sud_sub.MRN)
                pop['sud'] = pop['sud'].astype(int)

                depression_sub = depression_icd[depression_icd.DIAGNOSIS_DATE<followup]
                pop['depression'] = pop.MRN.isin(depression_sub.MRN)
                pop['depression'] = pop['depression'].astype(int)

                anxiety_sub = anxiety_icd[anxiety_icd.DIAGNOSIS_DATE<followup]
                pop['anxiety'] = pop.MRN.isin(anxiety_sub.MRN)
                                        ...
                                        #for oprx in [0,1]:
                                            ...
                                            if c>1:
                                                df = pd.DataFrame({
                                                    ...
                                                    'OR': np.exp(coefs),
                                                    '.025': np.exp(low_interval),
                                                    '.975': np.exp(high_interval),
                                                    'p': ps,
                                                ...
                                                df.to_csv(f'../voe_outputs/OUD/controlsNoOpioids/OPIOIDRX_voe_{beg_year}_{end_year}.csv', index=None)
...
#                                                         formula = f'{formula} + num_op'
                                            ...
                                            fifi.to_csv(f'../voe_outputs/OUD/controlsNoOpioids/OPIOIDRX_voe_{beg_year}_{end_year}_{c}.csv')
