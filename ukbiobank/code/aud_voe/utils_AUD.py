...
def import_data():
    ...
    # opi_prescrip['med_duration'] = (opi_prescrip.MEDICATION_END_DATE - opi_prescrip.issue_date).dt.days
    ...
    #Remove AUD from SUD because AUD is predictor
    #Remove tobacco-related DX from SUD because Smoking is being separately controlled for (refer to DAG)
    ...
    #remove AUD ICDs from sud_icd
    common_icd = set(sud_icd.icd_code).intersection(aud_icd.icd_code)
    sud_icd = sud_icd[~sud_icd.icd_code.isin(common_icd)]
    #remove TOBACCO ICDs from sud_icd
    common_icd = set(sud_icd.icd_code).intersection(tobacco_icd.icd_code)
    sud_icd = sud_icd[~sud_icd.icd_code.isin(common_icd)]
...
def initialize_empty_lists():
    ...
    hx_anxiety_col = []
    hx_mat_col = []
    control_opioid_rx_count_col = []
    # num_recs_covar_col = []
    # con_opi_prescrip_col = []
    return c,control_N,aud_N,control_mean_age,aud_mean_age,control_sd_age,aud_sd_age,control_perc_male,aud_perc_male,control_perc_female,\
        aud_perc_female,coefs,stderrs,ps,low_interval,high_interval,num_control_ncd,num_aud_ncd,followup_interval_col,beg_year_col,end_year_col,num_op_col,\
            ncd_thresh_col,hx_sickle_col,hx_hepc_col,hx_hiv_col,hx_aud_col,hx_tobacco_col,hx_sud_covar_col,hx_depression_col,hx_anxiety_col,hx_mat_col,control_opioid_rx_count_col

def opioid_prescriptions(begin, end, opi_prescrip):
    ...
    return mrn_opioid_counts#,mrn_opioids
...
def MAT(opi_prescrip, followup, pop):
    mat_df = opi_prescrip[opi_prescrip.drug_name.str.contains('METHADONE|BUPRENORPHINE|LOFEXIDINE')]
...
def update_results_csv(beg_year, end_year, c, control_N, opioid_N, control_mean_age, opioid_mean_age, control_sd_age, opioid_sd_age, control_perc_male,\
     opioid_perc_male, control_perc_female, opioid_perc_female, coefs, stderrs, ps, low_interval, high_interval, num_control_ncd, num_opioid_ncd, \
        ...
        ncd_thresh_col, hx_sickle_col, hx_hepc_col, hx_hiv_col, hx_tobacco_col, #hx_aud_col, 
        hx_sud_covar_col, #hx_mat_col,
    ...
    if c>1:
        df = pd.DataFrame({
                        ...
                        #'hx_MAT': hx_mat_col
                        # 'hx_depression':hx_depression_col,
...
def statistical_model(hx_tobacco, hx_sud_covar, outcome='ncd'): #hx_sickle, hx_hepc, hx_hiv,  #hx_aud, 
    ...
    #hx_mat, opioid_predictor='binary_exposure',
    ...
    # if hx_depression:
    #     formula = f'{formula} + C(depression)'
    # if hx_anxiety:
    #     formula = f'{formula} + C(anxiety)'
    # if control_opioid_rx_count:
    #     formula = f'{formula} + opioid_count'
    return formula
...
def append_data_to_lists(beg_year, end_year, control_N, opioid_N, control_mean_age, opioid_mean_age, control_sd_age, opioid_sd_age, control_perc_male,\
     opioid_perc_male, control_perc_female, opioid_perc_female, coefs, stderrs, ps, low_interval, high_interval, num_control_ncd, num_opioid_ncd,\
         ...
         ncd_thresh_col, hx_sickle_col, hx_hepc_col, hx_hiv_col, hx_tobacco_col, #hx_aud_col,\
             ...
             followup_interval, #num_op, 
             ncd_thresh, control_cohort, opioid_cohort,\
                 scalar_con_mean_age, scalar_opi_mean_age, scalar_con_sd_age, scalar_opi_sd_age, scalar_con_perc_male, scalar_opi_perc_male,\
                     scalar_con_perc_female, scalar_opi_perc_female, pop, hx_tobacco, #hx_sickle, hx_hepc, hx_hiv, hx_aud, 
                     hx_sud_covar,#hx_depression, hx_anxiety,control_opioid_rx_count,\
                        #hx_mat, 
                        res):
    ...
    # hx_depression_col.append(hx_depression)
    # hx_anxiety_col.append(hx_anxiety)
    # control_opioid_rx_count_col.append(control_opioid_rx_count)
...
def export_final_data_enrollment_period(new_line, beg_year, end_year, c, control_N, opioid_N, control_mean_age,
     opioid_mean_age, control_sd_age, opioid_sd_age, control_perc_male, opioid_perc_male, control_perc_female,
